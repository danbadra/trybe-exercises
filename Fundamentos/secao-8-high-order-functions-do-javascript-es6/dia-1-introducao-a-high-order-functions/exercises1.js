...
// Desenvolva uma HOF que retorne o resultado de um sorteio. Essa HOF irá gerar um número aleatório entre 1 e 5, recebendo como parâmetros o número apostado e uma função que checa se o número apostado é igual ao número sorteado. O retorno da sua HOF deve ser uma string (Ex: “Tente novamente” ou “Parabéns, você ganhou”).
    ...
    const confereNumero = (meuNumero, numero) => meuNumero === numero;

    const sorteioLoteria = (meuNumero, callback) => {
        const numero = Math.floor((Math.random() * 5) + 1);
        console.log(`O número sorteado foi ${numero}.`);
        return callback(meuNumero, numero) ? 'Parabéns, você ganhou!' : 'Tente novamente.';
    };

    // console.log(sorteioLoteria(1, confereNumero));
...
// Ao final, a HOF deve retornar o total de pontos obtidos através das respostas fornecidas pela pessoa estudante.
    ...
    const countPoints = (gabarito, respostas, callback) => {
        let contador = 0;

        for (let index = 0; index < gabarito.length; index += 1) {
            const actionReturn = callback(gabarito[index], respostas[index]);
            contador += actionReturn;
          }
          return `Resultado final: ${contador} pontos`;

        // gabarito.forEach((correcao) => {
        //     correcao = callback(gabarito, respostas);
        //     contador += correcao;
        // });
    ...
    console.log(countPoints(RIGHT_ANSWERS, STUDENT_ANSWERS, correcao));
